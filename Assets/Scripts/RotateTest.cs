...
public class RotateTest : MonoBehaviour
{
    ...
    //traceTarget 프로퍼티 정의(gettet, settet)
    ...
    void Start()
    {
        ...
        if (group != null)
        {
            //WayPoint 하위에 있는 모든 Transform 컴포넌트를 추출한 후
    ...
    void Update()
    {
        ...
        if(agent.isStopped == false)
        {
            ...
            petTransform.rotation = Quaternion.Slerp(petTransform.rotation, rot, Time.deltaTime * damping);
        }
        //순찰 모드가 아닐 경우 이후 로직을 수행하지 않음
        if (!_moving) return;


        //NavMeshAgent가 이동하고 있고 목적지에 도착했느지 여부를 계산
        if(agent.velocity.sqrMagnitude >= 0.2f *0.2f && agent.remainingDistance <=0.5f )
        {
            //다음 목적지의 배욜 첨자를 계산
            //nextIdx = ++nextIdx % wayPoints.Count;
            //extIdx = Random.Range(0, wayPoints.Count);
